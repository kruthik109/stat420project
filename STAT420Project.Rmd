...
#plot 2nd difference, acf, pacf
par(mfrow=c(2,2))
plot(diff(y, differences = 2), type='l', ylab = "2nd Difference of Confirmed Cases",  main = "2st Order Difference of Confirmed Cases")
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))

#plot 1st seasonal difference, acf, pacf
par(mfrow=c(2,2))
fin = diff(diff(y, differences = 2), 7)
plot(fin, type = 'l',  ylab = "1st Seasonal Difference, Confirmed Cases",  main = "1st Seasonal Diff, Confirmed Cases")
acf(fin)
pacf(fin)
```
...
par(mfrow=c(2,2))
plot(confirmed.arima1$residuals, ylab = 'residuals', main = "Model 1 (0,2,1)x(0,1,1)7")
...
confirmed.arima2 = arima(y, order=c(0, 2,1), seasonal=list(order=c(0, 1,2), period=7))

plot(confirmed.arima2$residuals, ylab = 'residuals',  main = "Model 2 (0,2,2)x(0,1,1)7")
acf(confirmed.arima2$residuals, lag.max  = 50)
pacf(confirmed.arima2$residuals, lag.max  = 50)

qqnorm(confirmed.arima2$residual)
qqline(confirmed.arima2$residual)

confirmed.arima2$aic
Box.test(confirmed.arima2$residuals, lag = 20, fitdf=1, type="Ljung")

#Model 3
par(mfrow=c(2,2))
confirmed.arima3 = arima(y, order=c(1, 2,1), seasonal=list(order=c(0, 1,2), period=7))

plot(confirmed.arima3$residuals, ylab = 'residuals', main ="Model 3 (1,2,1)x(0,1,2)7" )
...
Box.test(confirmed.arima3$residuals, lag = 20, fitdf=2, type="Ljung")
```
We found that model 2 would be the best which is the ARIMA(0,2,1)x(0,1,2)7. Next, we take a closer look at the overall forecasting using the model. 

```{r RQ3 forecast}
#Forecasting with model 1 confirmed cases
forecast=predict(confirmed.arima12, n.ahead =50)
#overall forecast
plot(y, xlim = c(0,700), ylim = c(0, 262659973), type='l')
lines(forecast$pred, col="green")
#closer look
sarima.for(world$Confirmed, 50, 0,2,1,0,1,2,7)
```

The overall conclusion regarding this question is the confirmed cases can be forecasted using an ARIMA(0,2,1)x(0,1,1)7 model, because from the ACF the residuals are uncorrelated with the exception of 2 outliers. The data follows a mostly normal distribution and the AIC is the lowest at 15882.59.
